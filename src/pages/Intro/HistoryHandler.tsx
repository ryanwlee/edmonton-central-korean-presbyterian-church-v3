...
import { timeTableHandler } from "@/pages/Intro/Handler";
...
const GreenRound = Styled.img<{ percent: number; containerHeight: number }>`
  position: absolute;
...
const DynamicYear = Styled.div<{ percent: number; containerHeight: number }>`
  position: absolute;
  height: 30px;
...
const HistoryHandler = (props: HistoryHandlerProps) => {
  ...
  const updateHeight = () => {
    if (timelineRef.current) {
      setContainerHeight(timelineRef.current.clientHeight || 40);
    }
  };
